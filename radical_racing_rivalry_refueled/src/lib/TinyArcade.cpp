...
uint8_t getJoystickState() {
  ...
  if (arcadeVersion == 1) {
    int analogVal = analogRead(TinyArcadePinY);
    if (analogVal < 512 - 250) {
      rVal |= TAJoystickUp;
    }
    else if (analogVal > 512 + 250) {
      rVal |= TAJoystickDown;
    }
    analogVal = analogRead(TinyArcadePinX);
    if (analogVal > 512 + 250) {
    ...
    if (!digitalRead(TinyArcadePinUp)) {
      rVal |= TAJoystickUp;
    }
    if (!digitalRead(TinyArcadePinDown)) {
      rVal |= TAJoystickDown;
    }
    if (!digitalRead(TinyArcadePinLeft)) {
      rVal |= TAJoystickLeft;
    }
    if (!digitalRead(TinyArcadePinRight)) {
      rVal |= TAJoystickRight;
    }
  }
  return rVal;
